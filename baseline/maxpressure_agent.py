...
class MaxPressureAgent(Agent):
    ...
    def choose_action(self, count, state):
        ...
        # #
        # phase_1 = max((np.sum(state.coming_vehicles[:, :3]) + np.sum(self.state.coming_vehicles[:, 6:9]) - \
        #                np.sum(state.leaving_vehicles[:, :3]) - np.sum(state.leaving_vehicles[:, 6:9])), 0)
        # phase_2 = max((np.sum(state.coming_vehicles[:, 3:6]) + np.sum(state.coming_vehicles[:, 9:12]) - \
        #                np.sum(state.leaving_vehicles[:, :3]) - np.sum(state.leaving_vehicles[:, 6:9])), 0)
